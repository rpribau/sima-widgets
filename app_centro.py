...
def run():
    ...
    model_path = f"results/{contaminant}_centro/model_{contaminant}_centro.pkl"
    try:
        with open(model_path, "rb") as model_file:
            model = pickle.load(model_file)
            st.success(f"Modelo para {contaminant} cargado exitosamente.")
    except FileNotFoundError:
        st.error(f"No se encontró el modelo para {contaminant} en Centro.")
        return
    ...
    last_X = X.iloc[-120:]
    real_values = df[f"{contaminant}_centro"].iloc[-240:]
    ...
    fig.update_layout(title=f"Pronóstico para {contaminant} en Centro", xaxis_title="Fecha", yaxis_title="Concentración", template='plotly_white')
