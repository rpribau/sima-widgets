...
def run():
    ...
    model_path = f"results/{contaminant}_noroeste2/model_{contaminant}_noroeste2.pkl"
    try:
        with open(model_path, "rb") as model_file:
            model = pickle.load(model_file)
            st.success(f"Modelo para {contaminant} cargado exitosamente.")
    except FileNotFoundError:
        st.error(f"No se encontró el modelo para {contaminant} en Noroeste2.")
        return
    ...
    last_X = X.iloc[-120:]
    real_values = df[f"{contaminant}_noroeste2"].iloc[-240:]
    ...
    fig.update_layout(title=f"Pronóstico para {contaminant} en Noroeste2", xaxis_title="Fecha", yaxis_title="Concentración", template='plotly_white')
